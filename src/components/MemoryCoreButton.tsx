...
interface MemoryCoreButtonProps {
  onClick?: () => void;
  className?: string;
}

export const MemoryCoreButton: React.FC<MemoryCoreButtonProps> = ({
  onClick,
  className = "",
}) => {
  const [isHovered, setIsHovered] = React.useState(false);
  const rawId = React.useId().replace(/:/g, "");
  const gradientId = `memory-core-gradient-${rawId}`;
  const clipId = `memory-core-clip-${rawId}`;

  return (
    <button
      type="button"
      aria-label="Open Memory List"
      className={`pointer-events-auto cursor-pointer flex items-center justify-center relative w-11 h-11 transition-all duration-200 bg-transparent border-0 p-0 shrink-0 ${className}`}
